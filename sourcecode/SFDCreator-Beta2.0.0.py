 ...
 if os.path.isfile(ffmpeg_exe_path):
  ...
  ffmpeg_location_int = 1
   ...
   if ffmpegmissing_message:
    ...
    master.title("SFDCreator Beta 2.0.0 (PREVIEW MODE)")
...
master = tk.Tk()
master.geometry("600x450"), master.title("SFDCreator Beta 2.0.0"), master.resizable(False, False)#, master.iconbitmap("resource/icon/sfdextractor.ico")
...
def createSFD():
   ...
   def move_or_remove_sfdcreation_files():
     if IntVar.get(keepAVI) == 1:
      ...
      if os.path.exists(source_fileavi):
       dest_fileavi = os.path.join(dirPath.get(), 'AVIconvert.avi')
       shutil.move(source_fileavi, dest_fileavi)
     else:
      os.remove('AVIconvert.avi')
    
     if IntVar.get(keepMPEG) == 1:
      source_filempg = os.path.join(currentdir, f'newvideo.{outputmpegextension.get()}')
      if os.path.exists(source_filempg):
       dest_filempg = os.path.join(dirPath.get(), f'newvideo.{outputmpegextension.get()}')
     ...
     if IntVar.get(keepsfdmuxlog) == 1:
      ...
      if os.path.exists(source_filelog):
       dest_filelog = os.path.join(dirPath.get(), 'log.txt')
       shutil.move(source_filelog, dest_filelog)
     else:
      if os.path.isfile('log.txt'): #Check for log.txt, since SFD_Muxer (used for non-SofdecStream2 files) doesn't make a log file.
       os.remove('log.txt')

     if IntVar.get(keepWAV) == 1:
      source_filea1 = os.path.join(currentdir, f'track1.{audiofiletype.get()}')
      if os.path.exists(source_filea1):
       dest_fileaud1 = os.path.join(dirPath.get(), f'track1.{audiofiletype.get()}')
     ...
     if IntVar.get(keepADX) == 1:
      source_filea1_adx = os.path.join(currentdir, 'track1.adx')
      if os.path.exists(source_filea1_adx):
       dest_fileaud1_adx = os.path.join(dirPath.get(), 'track1.adx')
       shutil.move(source_filea1_adx, dest_fileaud1_adx)
      else:
       print(f"track1.adx not found, unable to move to destination directory.")
     else:
      if os.path.exists(f'track1.adx'):
       os.remove(f'track1.adx')

     if IntVar.get(keepWAV) == 1:
      source_filea2 = os.path.join(currentdir, f'track2.{audiofiletype.get()}')
      if os.path.exists(source_filea2):
       dest_fileaud2 = os.path.join(dirPath.get(), f'track2.{audiofiletype.get()}')
       shutil.move(source_filea2, dest_fileaud2)
      else:
     ...
     if IntVar.get(keepADX) == 1:
      source_filea2_adx = os.path.join(currentdir, 'track2.adx')
      if os.path.exists(source_filea2_adx):
       dest_fileaud2_adx = os.path.join(dirPath.get(), 'track2.adx')
       shutil.move(source_filea2_adx, dest_fileaud2_adx)
      else:
       print(f"track2.adx not found, unable to move to destination directory.")
     else:
      if os.path.exists('track2.adx'):
       os.remove('track2.adx')

     if IntVar.get(keepWAV) == 1 and addaudiotracks.get() == 1:
      source_filea3 = os.path.join(currentdir, f'track3.{audiofiletype.get()}')
      if os.path.exists(source_filea3):
       dest_fileaud3 = os.path.join(dirPath.get(), f'track3.{audiofiletype.get()}')
       shutil.move(source_filea3, dest_fileaud3)
      else:
     ...
     if IntVar.get(keepADX) == 1 and addaudiotracks.get() == 1:
      source_filea3_adx = os.path.join(currentdir, 'track3.adx')
      if os.path.exists(source_filea3_adx):
       dest_fileaud3_adx = os.path.join(dirPath.get(), 'track3.adx')
       shutil.move(source_filea3_adx, dest_fileaud3_adx)
      else:
       print(f"track3.adx not found, unable to move to destination directory.")
     else:
      if os.path.exists('track3.adx'):
       os.remove('track3.adx')

     if IntVar.get(keepWAV) == 1 and addaudiotracks.get() == 1:
      source_filea4 = os.path.join(currentdir, f'track4.{audiofiletype.get()}')
      if os.path.exists(source_filea4):
       dest_fileaud4 = os.path.join(dirPath.get(), f'track4.{audiofiletype.get()}')
       shutil.move(source_filea4, dest_fileaud4)
      else:
     ...
     if IntVar.get(keepADX) == 1 and addaudiotracks.get() == 1:
      source_filea4_adx = os.path.join(currentdir, 'track4.adx')
      if os.path.exists(source_filea4_adx):
       dest_fileaud4_adx = os.path.join(dirPath.get(), 'track4.adx')
       shutil.move(source_filea4_adx, dest_fileaud4_adx)
      else:
       print(f"track4.adx not found, unable to move to destination directory.")
     else:
      if os.path.exists('track4.adx'):
       os.remove('track4.adx')

     if IntVar.get(keepADX) == 1:
      source_filea1_sfa = os.path.join(currentdir, 'track1.sfa')
      if os.path.exists(source_filea1_sfa):
       dest_fileaud1_sfa = os.path.join(dirPath.get(), 'track1.sfa')
       shutil.move(source_filea1_sfa, dest_fileaud1_sfa)
      else:
       print(f"track1.sfa not found, unable to move to destination directory.")
     else:
      if os.path.exists('track1.sfa'):
       os.remove('track1.sfa')

     if IntVar.get(keepADX) == 1:
      source_filea2_sfa = os.path.join(currentdir, 'track2.sfa')
      if os.path.exists(source_filea2_sfa):
       dest_fileaud2_sfa = os.path.join(dirPath.get(), 'track2.sfa')
       shutil.move(source_filea2_sfa, dest_fileaud2_sfa)
      else:
       print(f"track2.sfa not found, unable to move to destination directory.")
     else:
      if os.path.exists('track2.sfa'):
       os.remove('track2.sfa')

     if IntVar.get(keepADX) == 1 and addaudiotracks.get() == 1:
      source_filea3_sfa = os.path.join(currentdir, 'track3.sfa')
      if os.path.exists(source_filea3_sfa):
       dest_fileaud3_sfa = os.path.join(dirPath.get(), 'track3.sfa')
       shutil.move(source_filea3_sfa, dest_fileaud3_sfa)
      else:
       print(f"track3.sfa not found, unable to move to destination directory.")
     else:
      if os.path.exists('track3.sfa'):
       os.remove('track3.sfa')
     
     if IntVar.get(keepADX) == 1 and addaudiotracks.get() == 1:
      source_filea4_sfa = os.path.join(currentdir, 'track4.sfa')
      if os.path.exists(source_filea4_sfa):
       dest_fileaud4_sfa = os.path.join(dirPath.get(), 'track4.sfa')
       shutil.move(source_filea4_sfa, dest_fileaud4_sfa)
      else:
       print(f"track4.sfa not found, unable to move to destination directory.")
     else:
      if os.path.exists('track4.sfa'):
       os.remove('track4.sfa')
   ...
   if file_size_kb < min_file_size:
     ...
     videocheckcommand = f"ffprobe -v error -show_entries stream=codec_type -of default=noprint_wrappers=1 file.sfd -select_streams v:0"
     ...
     if videocheckcmd_output.returncode == 0:
       ...
       source_filesfd = os.path.join(os.getcwd(), SFDname_withextension)
       if os.path.exists(source_filesfd):
        dest_filesfd = os.path.join(dirPath.get(), SFDname_withextension)
