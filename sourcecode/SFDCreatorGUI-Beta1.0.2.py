...
if os.path.isfile('ffmpeg.exe'):
 print("FFmpeg.exe found (in SofdecVideoTools folder)")
else:
 if shutil.which('ffmpeg.exe'):
  print("FFmpeg.exe found (in PATH)")
 else:
  tk.messagebox.showerror('Error', "FFmpeg could not be found. FFmpeg is required to run these programs. If you don't already have FFmpeg, please refer to the Documentation to install it.")
  sys.exit()

if os.path.isfile('ffprobe.exe'):
 print("FFprobe.exe found (in SofdecVideoTools folder)")
else:
 if shutil.which('ffprobe.exe'):
  print("FFprobe.exe found (in PATH)")
 else:
  tk.messagebox.showerror('Error', "FFprobe could not be found. FFprobe is required to run these programs. If you don't already have FFmpeg, please refer to the Documentation to install it.")
  sys.exit()

master = tk.Tk()
master.geometry("600x450"), master.title("SFDCreator Beta 1.0.2"), master.resizable(False, False)#, master.iconbitmap("resource/icon/sfdextractor.ico")
...
def SFDmux():
    ...
    if os.path.exists(filePathvideo.get()):
     copiedvideo = os.path.join(os.getcwd(), 'video' + os.path.splitext(videofile)[1])
     shutil.copy(videofile, copiedvideo)
     newvideo = 'video' + os.path.splitext(videofile)[1]
     os.rename(copiedvideo, newvideo)
     filePathvideo.set(newvideo)
     if UseVideoAudio.get() == 1:
      filePathaudt1.set(newvideo)
     if UseTrack1forTrack2.get() == 1:
      filePathaudt2.set(filePathaudt1.get())

     def onvideofilechange(*args):
      previousvideovalue = filePathvideo.get()
      filePathvideo.trace('w', onvideofilechange)
    ...
    command=f"ffmpeg -y {ffmpegloglevel.get()} -i {newvideo} {ffmpegspeedpreset.get()} -crf 01 -b:v 30000000 {framerate.get()} {resolution.get()} {vratio.get()} AVIconvert.avi"
    os.system(command)
    ...
    min_file_size = 50
    file_size_kb = os.path.getsize('newSFD.sfd') // 50
    ...
    #print("Moving file(s) to output directory...")
    ...
    os.remove(newvideo)
    filePathvideo.set(previousvideovalue.get())
