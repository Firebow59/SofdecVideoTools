 ...
 if os.path.isfile(ffmpeg_exe_path):
  ...
  ffmpeg_location_int = 1
   ...
   if ffmpegmissing_message:
    ...
    master.title("SFDCreator Beta 2.0.1 (PREVIEW MODE)")
...
master = tk.Tk()
master.geometry("600x450"), master.title("SFDCreator Beta 2.0.1"), master.resizable(False, False)#, master.iconbitmap("resource/icon/sfdextractor.ico")
   ...
   def move_or_remove_sfdcreation_files():
     ...
     if IntVar.get(keepMPEG) == 1:
      ...
      if os.path.exists(source_filempg):
       ...
       if os.path.isfile('newvideo.mpeg'): #Fix to delete this if video is converted to M1V
